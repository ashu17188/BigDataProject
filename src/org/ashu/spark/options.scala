...
abstract case class OptionParser[C](programName: String) {
  ...
  protected def makeDef[A: Read](kind: OptionDefKind, name: String): OptionDef[A, C] =
   ...
   def updateOption[A: Read](option: OptionDef[A, C]): OptionDef[A, C] = {
...
class OptionDef[A: Read, C](
 ...
 def copy(
  ...
  def validateConfig(f: C => Either[String, Unit]) =
    _parser.updateOption(copy(_configValidations = _configValidations :+ f))
  def parent(x: OptionDef[_, C]): OptionDef[A, C] =
  ...
  def children(xs: OptionDef[_, C]*): OptionDef[A, C] = {
   ...
   val kind: OptionDefKind = _kind
 val id: Int = _id
  val name: String = _name
  def callback: (A, C) => C = _action
   def getMinOccurs: Int = _minOccurs
  def getMaxOccurs: Int = _maxOccurs
  def shortOptOrBlank: String = _shortOpt getOrElse("")
  def hasParent: Boolean = _parentId.isDefined
 def getParentId: Option[Int] = _parentId
   def isHidden: Boolean = _isHidden
  def checks: Seq[C => Either[String, Unit]] = _configValidations
 ...
 def longOptTokens(arg: String): Int =
   ...
   def tokensToRead(i: Int, args: Seq[String]): Int =
    ...
    def usageTwoColumn(col1Length: Int): String = {
